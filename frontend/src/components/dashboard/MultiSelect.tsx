...
export function MultiSelect({
  ...
  return (
    <LabeledControl action={labelAction} htmlFor={id} label={label}>
      <Combobox
        items={options}
        multiple
      ...
      >
        <ComboboxInputGroup>
                ...
                <>
                  {values.map(({ label: optionLabel, value }) => (
                    <ComboboxChip
                      ...
                      className="flex max-w-full items-center gap-1 rounded-md border border-primary-light bg-primary-subtle px-1.5 py-0.5 text-xs/tight font-normal"
                      key={value}
                    >
                      {codeLabels ? (
          ...
          <ComboboxClear
            ...
            className="grid w-10 flex-none place-items-center self-stretch rounded-r-md border-0 border-l border-solid border-l-border bg-transparent p-0 text-muted-foreground hover:bg-muted hover:text-foreground"
            ...
            {(option: AutocompleteOption) => (
              <ComboboxItem key={option.value} value={option}>
                ...
                {option.detail !== undefined && (
                  <small className="text-xs text-muted-foreground">{option.detail}</small>
                )}
              </ComboboxItem>
            )}
          </ComboboxList>
