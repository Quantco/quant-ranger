import { hasPullRequest, parseGitHubRepository } from '@/lib/github//pull-request'
...
export const createGitHubTokenUrl = (report: UpdaterReportSnapshot): string | null => {
  ...
  if (!URL.canParse(report.github_api_url)) {
    return null
  }

  const apiHost = new URL(report.github_api_url).hostname
  if (apiHost !== 'api.github.com' && !(apiHost.startsWith('api.') && apiHost.endsWith('.ghe.com'))) {
    return null
  }
