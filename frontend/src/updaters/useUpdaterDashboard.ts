...
const resultId = (result: UpdaterReportResult, index: number) =>
  `${result.repository}-${result.target ?? ''}-${result.pull_request ?? ''}-${index}`
...
export const useUpdaterDashboardController = (report: UpdaterReportSnapshot) => {
  ...
  const {
    resetState: clearAllState,
    setState,
    state
  } = useUrlState({ defaultState: DEFAULT_UPDATER_DASHBOARD_STATE, parse })
  ...
  const visibility = useStateColumnVisibility(state, setState, {
    columns: UPDATER_RESULT_COLUMN_IDS,
    pinned: UPDATER_REPOSITORY_COLUMN
  })
